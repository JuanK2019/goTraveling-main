...
export default function ChangeDisplayNameForm({ displayName, setShowModal, toastRef, setReloadUser }) {
    ...
    const onSubmit = async() => {
        ...
        if (!result.statusResponse) {
            setError("Error al actualizar nombres y apellidos, por favor intenta más tarde.")
            return
        }

        setReloadUser(true)
        toastRef.current.show("Se han actualizado los nombres y apellidos.", 3000)
        setShowModal(false)
    }
    ...
    return (
        <View style={styles.view}>
            ...
            />
            <Button
                title="Actualizar Nombres y Apellidos"
